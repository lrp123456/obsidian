...
我需要修改整体的爬取逻辑，之前的逻辑是搜索关键词后直接滚动爬取，通过详情页获取详细数据，最终输出结果，这个结果爬取的数据实际上是比较低质的，我发现我们可以通过点击搜索到的某一个详情页后点击详情页中的相似推荐，相似推荐的数据是比较好的，且越好数据的pin的相似推荐数据也比较好，所以我想我们可不可以通过搜索一个关键词后不滚动，点击随机一个详情页，然后再点击
##### 7.7.1 基本概念
